...
/**
 * Generates the output for easyocd questions.
 *
 * @copyright  2014 onwards Carl LeBlond
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_easyocd_renderer extends qtype_renderer {
    public function formulation_and_controls(question_attempt $qa, question_display_options $options) {
        ...
        if ($options->correctness) {
            $result .= html_writer::tag('input', '', array(
                ...
                'type' => 'button',
                'value' => get_string('my_response', 'qtype_easyocd')
            ));
            $result .= html_writer::tag('input', '', array(
                'id' => 'corresponse' . $uniqid,
                'type' => 'button',
                'value' => get_string('correct_answer', 'qtype_easyocd')
            ));
            $this->page->requires->js_init_call('M.qtype_easyocd.showmyresponse', array(
                $CFG->version,
                $uniqid
            ));
            $this->page->requires->js_init_call('M.qtype_easyocd.showcorresponse', array(
                $CFG->version,
                $uniqid
            ));
        }
        $toreplaceid = 'applet' . $uniqid;
        $toreplace   = html_writer::tag('div', get_string('enablejavaandjavascript', 'qtype_easyocd'), array(
            'id' => $toreplaceid, 'class' => 'easyocd resizable'
        ));
        $answerlabel = html_writer::tag('span', get_string('answer', 'qtype_easyocd', ''), array(
                'class' => 'answerlabel'
            ));
        ...
        'src'=>$OUTPUT->pix_url('chemaxon', 'qtype_easyocd'),
    ...
    protected function require_js($toreplaceid, question_attempt $qa, $readonly, $correctness, $uniqid) {
        ...
        $topnode        = 'div.que.easyocd#q' . $qa->get_slot();
        $feedbackimage  = '';
        if ($correctness) {
            $feedbackimage = $this->feedback_image($this->fraction_for_last_response($qa));
        }
        $name             = 'EASYOCD' . $uniqid;
        $appletid         = 'easyocd' . $uniqid;
        $strippedanswerid = "stripped_answer" . $uniqid;
        $PAGE->requires->js_init_call('M.qtype_easyocd.insert_easyocd_applet', array(
            $toreplaceid,
            $name,
            $appletid,
            $topnode,

...
/**
 * Defines the editing form for the easyocd question type.
 *
 * @package    qtype
 * @subpackage easyocd
...
class qtype_easyocd_edit_form extends qtype_shortanswer_edit_form {
    protected function definition_inner($mform) {
        global $PAGE, $CFG;
        $PAGE->requires->css(new moodle_url($protocol . $_SERVER['HTTP_HOST'] . $marvinjspath . '/ChemDoodleWeb.css'));
        $marvinjsconfig = get_config('qtype_easyocd_options');
        ...
        $easyocdhomeurl = 'http://www.chemaxon.com';
        $mform->addElement('html', html_writer::link($easyocdhomeurl, get_string('easyocdeditor', 'qtype_easyocd')));
        $mform->addElement('html', html_writer::empty_tag('br'));
        $mform->addElement('html', html_writer::tag('span', get_string('author', 'qtype_easyocd'), array(
            'class' => 'easyocdauthor'
        )));
        $mform->addElement('html', html_writer::end_tag('small'));
        $mform->addElement('html', html_writer::end_tag('div'));
        $mform->addElement('html', html_writer::end_tag('div'));
        $marvinconfig = get_config('qtype_easyocd_options');
        $marvinpath   = $marvinconfig->path;
        $PAGE->requires->js_init_call('M.qtype_easyocd.insert_applet', array(
            $CFG->wwwroot,
            $marvinpath
        ));
        $this->add_per_answer_fields($mform,
            get_string('answerno', 'qtype_easyocd', '{no}'), question_bank::fraction_options());
        $this->add_interactive_settings();
        $PAGE->requires->js_init_call('M.qtype_easyocd.init_getanswerstring', array(
            $CFG->version
        ));
        $PAGE->requires->js_init_call('M.qtype_easyocd.init_viewanswerstring', array(
            $CFG->version
        ));
        $PAGE->requires->js_init_call('M.qtype_easyocd.init_viewanswerstring', array(
            $CFG->version
    ...
    protected function get_per_answer_fields($mform, $label, $gradeoptions,
        ...
        $viewbutton = $mform->createElement('button', 'view',
            get_string('view', 'qtype_easyocd'), 'class = id_view');
        array_splice($repeated, 1, 0, array($viewbutton));
        $insertbutton = $mform->createElement('button', 'insert',
            get_string('insertfromeditor', 'qtype_easyocd'), $scriptattrs);
